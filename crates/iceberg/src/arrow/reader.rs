...
/// Maximum concurrency for parallel byte range fetching.
/// This limits the number of concurrent HTTP requests when reading column chunks.
const MAX_BYTE_RANGE_CONCURRENCY: usize = 32;
...
impl<R: FileRead> AsyncFileReader for ArrowFileReader<R> {
    ...
    /// Fetch multiple byte ranges in parallel.
    ///
    /// This improves I/O performance by executing concurrent HTTP requests for column chunks
    /// instead of sequential reads. Concurrency is capped at [`MAX_BYTE_RANGE_CONCURRENCY`]
    /// or the available system parallelism, whichever is smaller.
    fn get_byte_ranges(
        &mut self,
        ...
        let reader = &self.r;
        async move {
            let concurrency = available_parallelism().get().clamp(1, MAX_BYTE_RANGE_CONCURRENCY);
            ...
            stream::iter(reads).buffered(concurrency).try_collect().await
        }
        .boxed()
    }
