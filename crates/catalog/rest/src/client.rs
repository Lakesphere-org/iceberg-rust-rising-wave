...
impl HttpClient {
    ...
    pub fn new(cfg: &RestCatalogConfig) -> Result<Self> {
        ...
        let base_client = Client::builder()
            ...
            .build()
            .map_err(|e| Error::new(ErrorKind::Unexpected, "Failed to build HTTP client").with_source(e))?;
    ...
    pub fn update_with(self, cfg: &RestCatalogConfig) -> Result<Self> {
        ...
        // Re-add SigV4 middleware if enabled
        if cfg.sigv4_enabled() {
            let mut sigv4_middleware = crate::middleware::sigv4::SigV4Middleware::new(
                &cfg.base_url(),
                cfg.signing_name().as_deref().unwrap_or("glue"),
    ...
    async fn authenticate(&self, req: &mut Request) -> Result<()> {
        ...
        let auth_resp = self.client.execute(auth_req).await.map_err(|e| {
            Error::new(ErrorKind::Unexpected, "Failed to execute request")
    ...
    pub async fn query_catalog(&self, mut request: Request) -> Result<Response> {
        ...
        Ok(self.client.execute(request.try_clone().unwrap()).await.map_err(|e| {
            Error::new(ErrorKind::Unexpected, "Failed to execute request")
                .with_context("url", request.url().to_string())
                .with_source(e)
        })?)
    }
}
