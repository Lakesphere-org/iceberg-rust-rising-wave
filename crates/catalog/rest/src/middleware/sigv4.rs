...
impl Middleware for SigV4Middleware {
    async fn handle(
        ...
        if !req.url().as_str().starts_with(&self.catalog_uri) {
            println!("URL doesn't match catalog URI, skipping SigV4");
        ...
        let config = self
            ...
            .get_or_init(|| async {
                    ...
                    (&self.access_key_id, &self.secret_access_key)
                {
                    ...
                    println!("No explicit AWS credentials provided, using default credential chain");
                }
        ...
        for header in new_headers.into_iter() {
            ...
            //
            // if header.name().as_str() == "authorization" {
            //     println!("Setting Authorization header: {}", header.value());
            // }
